...
for line in f:
	...
	latency[d_type][n_participants].append(float(l[3]))
	tail_latency[d_type][n_participants].append(float(l[4]))

plt.figure(figsize=(4,3))
plt.plot(participants, [latency["DBO"][x] for x in participants],color='C0', label="DBO", marker='D', markersize=5)
plt.plot(participants, [latency["MaxRTT"][x] for x in participants] ,color='C2', marker='^', markersize=5)
plt.ylim(bottom=0)
plt.xticks(range(10,100,20))
plt.xlabel("# Participants", fontsize =14)
plt.ylabel(r"Latency $(\mu s)$", fontsize=14)
plt.legend(bbox_to_anchor=(-0.1, 1.02, 1.1, 1.02), loc=4, ncol=1, mode="expand", borderaxespad=0., prop={'size': 14}, frameon=False)
plt.tight_layout()
...
plt.figure(figsize=(4,3))
plt.plot(participants, [tail_latency["DBO"][x] for x in participants], color='C0', marker='D', markersize=5)
plt.plot(participants, [tail_latency["MaxRTT"][x] for x in participants], color='C2', marker='^', label="Max-RTT", markersize=5)
plt.legend(bbox_to_anchor=(-0.1, 1.02, 1.1, 1.02), loc=3, ncol=1, mode="expand", borderaxespad=0., prop={'size': 14}, frameon=False)
plt.ylim(bottom=0)
plt.xticks(range(10,100,20))
plt.xlabel("# Participants", fontsize =14)
plt.ylabel(r"Latency $(\mu s)$", fontsize=14)
...
plt.scatter(latency["DBO"][10], [1], label="DBO, 10 MPs", marker='d', s=100, c="red", zorder=2)
plt.scatter(latency["DBO"][60], [1], label="DBO, 60 MPs", marker='d', s=100, c="green", zorder=2)
...
plt.scatter(tail_latency["DBO"][10], [1], label="DBO, 10 MPs", marker='d', s=100, c="red", zorder=2)
plt.scatter(tail_latency["DBO"][60], [1], label="DBO, 60 MPs", marker='d', s=100, c="green", zorder=2)
plt.xlim(left=0)
plt.legend(prop={'size': 14})
plt.tight_layout()
